...
node verify/preload-probe.cjs [port]   # footage is buffered before the cut that needs it and parked on that cut's own first frame, a take on screen is not mounted twice, the pool stays small, and a take whose file is on this device is never streamed back out of its uploaded url - defaults to 42216
